...
class TimelineContainer(Gtk.Grid, Zoomable, Loggable):
    ...
    def _insertClipsAndAssets(self, objects, position, layer):
        if self.bTimeline is None:
        ...
        for object in objects:
            if isinstance(object, GES.Clip):
                object.set_start(clip_position)
                layer.add_clip(object)
                duration = object.get_duration()
            elif isinstance(object, GES.Asset):
                if object.is_image():
                ...
                layer.add_asset(object,
                                ...
                                inpoint=0,
                                duration=duration,
                                track_types=object.get_supported_formats())
            else:
                raise TimelineError("Cannot insert: %s" % type(object))
