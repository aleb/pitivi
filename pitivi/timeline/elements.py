...
class KeyframeCurve(FigureCanvas, Loggable):
    ...
    def __maybeCreateKeyframe(self, event):
        ...
        if line_contains and not keyframe_existed:
            # TODO: Use self.__source.get_value(event.xdata) when https://bugzilla.gnome.org/show_bug.cgi?id=756950 is fixed.
            value = event.ydata
    ...
    def __setTooltip(self, event):
        if event.xdata:
            # TODO: Use self.__source.get_value(int(event.xdata)) when https://bugzilla.gnome.org/show_bug.cgi?id=756950 is fixed.
            value = event.ydata
