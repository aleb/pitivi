...
class Pipeline(GES.Pipeline, SimplePipeline):
    ...
    def stepFrame(self, framerate, frames_offset):
        ...
        if self.playing():
            return
        if frames_offset < 0:
            self.video_sink.seek(-1.0, Gst.Format.TIME, Gst.SeekFlags.NONE,
                                Gst.SeekType.NONE, 0,
                                Gst.SeekType.NONE, 0)
        step_event = Gst.Event.new_step(format=Gst.Format.BUFFERS,
                                        amount=abs(frames_offset),
                                        rate=1.0,
                                        flush=True,
                                        intermediate=False)
        print (11, step_event)
        assert(self.video_sink.send_event(step_event))
        print (111)

    def x(self, framerate, frames_offset):
        try:
            position = self.getPosition()
        except PipelineError:
            self.warning(
